const API_KEY = "ffeb0c7b0be84a61b6479ec632eef52d";
...
function fetchTopHeadlines() {
    ...
    fetch(apiUrl)
       ...
       .then(data => {
            const headlinesContainer = document.getElementById('cards-container');
            ...
            data.articles.forEach(article => {
                if (article.urlToImage) { // Check if the article has an image
                    ...
                    fillDataInCard(cardClone, article);
                    headlinesContainer.appendChild(cardClone);
                }
            });
        })
       .catch(error => {
